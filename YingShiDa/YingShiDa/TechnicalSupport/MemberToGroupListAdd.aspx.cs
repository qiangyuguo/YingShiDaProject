...
{
    public partial class MemberToGroupListAdd : PageBase
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(txtContentQuestion.Text.Trim()))
                {
                    Common.MessageBox.ShowLayer(this,"问题内容不能为空",2);
                    return;
                }
                if (string.IsNullOrEmpty(txtContentAnswer.Text.Trim()))
                {
                    Common.MessageBox.ShowLayer(this, "问题答案不能为空", 2);
                    return;
                }
                if (view_action == "notify")
                {
                    ...
                    if (cp != null)
                    {
                        ...
                        if (flag)
                        {
                            Common.MessageBox.ShowRedirect(this, "/TechnicalSupport/MemberToGroupList.aspx");
                        }
                    }
                }
                else
