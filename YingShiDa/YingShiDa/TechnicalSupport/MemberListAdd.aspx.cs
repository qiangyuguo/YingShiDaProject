...
{
    public partial class MemberListAdd : PageBase
    {
        ...
        public void BindData()
        {
            ...
            {
                Model.Service_Concept cp = Factory.GetExecution().SelectByID<Model.Service_Concept>(requestID);
                if (cp != null)
                {
                    if (cp.Language == 1)
                    {
            ...
            try
            {
                ...
                if (view_action == "notify")
                {
                    ...
                    if (cp != null)
                    {
                        ...
                        bool flag = Factory.GetExecution().Update<Model.Service_Concept>(cp);
                        if (flag)
                        {
                            Common.MessageBox.ShowRedirect(this, "/TechnicalSupport/MemberList.aspx");
                        }
                    }
                }
                else
                {
                    Model.Service_Concept cp = new Model.Service_Concept();
                    cp.Language = Convert.ToInt32(Request.Form["language"]);
                    cp.Title = txtTitle.Text;
                    cp.Content = XiangQing;
                    cp.CreatePeople = txtCreatePeople.Text;
                    ...
                    cp.Keywords = txtKeywords.Text.Trim();
                    cp.Description = txtDescription.Text.Trim();
                    bool flag = Factory.GetExecution().Add<Model.Service_Concept>(cp);
                    if (flag)
                    {
                        Common.MessageBox.ShowRedirect(this, "/TechnicalSupport/MemberList.aspx");
                    }
