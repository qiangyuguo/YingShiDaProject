...
{
    public partial class TalentConceptAdd : PageBase
    {
        ...
        public void BindData()
        {
            ...
            {
                Model.Talent_Concept cp = Factory.GetExecution().SelectByID<Model.Talent_Concept>(requestID);
                if (cp != null)
                {
                    if (cp.Language == 1)
                    {
            ...
            try
            {
                ...
                if (view_action == "notify")
                {
                    ...
                    if (cp != null)
                    {
                        ...
                        bool flag = Factory.GetExecution().Update<Model.Talent_Concept>(cp);
                        if (flag)
                        {
                            Common.MessageBox.ShowRedirect(this, "/HumanResources/TalentConcept.aspx");
                        }
                    }
                }
                else
                {
                    Model.Talent_Concept cp = new Model.Talent_Concept();
                    cp.Language = Convert.ToInt32(Request.Form["language"]);
                    cp.Title = txtTitle.Text;
                    cp.Content = XiangQing;
                    cp.CreatePeople = txtCreatePeople.Text;
                    ...
                    cp.Keywords = txtKeywords.Text.Trim();
                    cp.Description = txtDescription.Text.Trim();
                    bool flag = Factory.GetExecution().Add<Model.Talent_Concept>(cp);
                    if (flag)
                    {
                        Common.MessageBox.ShowRedirect(this, "/HumanResources/TalentConcept.aspx");
                    }
