...
{
    public partial class StaffStyleAdd : PageBase
    {
        ...
        public void BindData()
        {
            ...
            {
                Model.Staff_Presence cp = Factory.GetExecution().SelectByID<Model.Staff_Presence>(requestID);
                if (cp != null)
                {
                    if (cp.Language == 1)
                    {
            ...
            try
            {
                ...
                if (view_action == "notify")
                {
                    ...
                    if (cp != null)
                    {
                        ...
                        if (flag)
                        {
                            Common.MessageBox.ShowRedirect(this, "/HumanResources/StaffStyle.aspx");
                        }
                    }
                }
                else
                {
                    Model.Staff_Presence cp = new Model.Staff_Presence();
                    cp.Language = Convert.ToInt32(Request.Form["language"]);
                    cp.Title = txtTitle.Text;
                    cp.Content = XiangQing;
                    cp.CreatePeople = txtCreatePeople.Text;
