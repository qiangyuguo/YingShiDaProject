...
var popUpCal = {
    ...
    formatDate: function(day, month, year, hour, minute) {
        ...
        if (this.timeSeparators && this.timeSeparators.length > 1 && this.timeSeparators[0] && hour != null && minute != null) {
            ...
            dateString += this.timeSeparators[0] + ((hour >= 10) ? hour : ('0' + hour)) + this.timeSeparators[1] + ((minute >= 10) ? minute : ('0' + minute));
    ...
    修改：显示样式。修改者：Alpha 修改时间：2010-1-6
    修改：“小时”的显示格式是双位数 修改时间：2010-1-8
    */
    showCalendar: function() {
        ...
        if (this.timeSeparators.length > 1) {
            timeSelect += '<div class="select"><select id="calendar_hour" style="width:40px">';
            if (this.timeSeparators.length == 4) {
                ...
                for (var a = 0; a < 24; a++) {
                    if (this.selectedHour == a) timeSelect += '<option value="' + a + '" selected="selected">' + ((a >= 10) ? a : ('0' + a)) + '</option>';
                    else timeSelect += '<option value="' + a + '">' + ((a >= 10) ? a : ('0' + a)) + '</option>';
                }
            }

            timeSelect += '</select>时</div><div class="select" style=" width:10px;">' + this.timeSeparators[1] + '</div><div class="select"><select id="calendar_minute" style="width:40px">';
            for (var a = 0; a <= 59; a++) {
                if (this.selectedMinute == a) timeSelect += '<option value="' + a + '" selected="selected">' + ((a >= 10) ? a : ('0' + a)) + '</option>';
                else timeSelect += '<option value="' + a + '">' + ((a >= 10) ? a : ('0' + a)) + '</option>';
            }
            timeSelect += '</select>分</div>';
        ...
        if (!this.changeYear) {
            ...
            html += '<div class="select"><select id="calendar_newYear" style="width:52px">';
        ...
        if (!this.changeMonth) {
            ...
            html += '<div class="select"><select id="calendar_newMonth" style="width:50px">';
        ...
        html += timeSelect + '</div><table id="calendar" cellpadding="0" cellspacing="0"><thead>' +
    ...
    */
    ...
    showTime: function() {
        ...
        if (this.timeSeparators.length > 1) {
            ...
            timeSelect += '</select></div> <div class="select" style="color:#FFF;width:10px;">' + this.timeSeparators[1] + '</div> <div class="select"><select id="calendar_minute" style="width:40px"';
            for (var a = 0; a <= 59; a++) {
            ...
            timeSelect += '<div id="CLD_OK">确定</div>';
        }
        ...
        html += '<!--[if lte IE 6.5]><iframe src="javascript:false;" id="calendar_cover"></iframe><![endif]-->' +
			...
			(this.closeAtTop ? '' : '<div id="calendar_control" style="width:155px;"><a id="calendar_clear">' + this.clearText + '</a>' +

			'<a id="calendar_close">' + this.closeText + '</a></div>');
        // add calendar to element to calendar Div
        $('#calendar_div').empty().append("<div style='height:36px;background:#000;'>"+html+"</div>").show(this.speed);
