$(document).ready(function () {
    ...
    $(document).ajaxComplete(function (event, request, settings) {
        ...
        ///排序结束
        $("#divGridList ul:odd").not(".page ul").addClass("odd");
        $("#divGridList ul").not(":first").not(".page ul").hover(function () {
            $(this).addClass("on");
        }, function () {
            $(this).removeClass("on");
        });
        $("#divGridList table tr:even").not(":first").addClass("even");
        $("#divGridList table tr").not(":first").hover(function () {
            $(this).addClass("hover");
        }, function () {
            $(this).removeClass("hover");
        });
        //点击tr变色
        $("#divGridList").find("tr").not(":first").not("#tr_noSelect").click(function () {
            $("#divGridList").find("tr").removeClass("trSelected");
...
//输入框分页跳转
function GoToPage(max) {
    var page = $.trim($("#txtToPage").val());
    if ("" != page) {
...
function SignOn(message) {
    ...
    top.$.open({
        id: 'signon',
        url: '/signon.html',
        title: message + '-威佳二手车管理系统-易联信息技术',
        modal: true,
        drag: false,
