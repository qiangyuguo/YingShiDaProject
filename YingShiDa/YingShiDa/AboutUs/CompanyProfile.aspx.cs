        ...
        public void BindData()
        {
            ...
            try
            {
                if (dbm.Open())
                {
                    ...
                    DataTable dt = DAL.GetDataTable.GetList<Model.Company_Profile>(Title, startDate, endDate, AspNetPager2.CurrentPageIndex,AspNetPager2.PageSize, out pageCount, out rowCount, dbm);
                    if (null != dt)
                    {
                ...
                switch (e.CommandName)
                {
                    case "Delete":
                        try
                        {
                            ...
                            {
                                if (Factory.GetExecution().Delete<Model.Company_Profile>(ID))
                                {
