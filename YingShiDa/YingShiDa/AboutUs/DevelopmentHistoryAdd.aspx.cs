...
{
    public partial class DevelopmentHistoryAdd : PageBase
    {
        ...
        public void BindData()
        {
            ...
            {
                Model.Company_History cp = Factory.GetExecution().SelectByID<Model.Company_History>(requestID);
                if (cp != null)
                {
                    if (cp.Language == 1)
                    {
            ...
            try
            {
                ...
                if (view_action == "notify")
                {
                    ...
                    if (cp != null)
                    {
                        ...
                        bool flag = Factory.GetExecution().Update<Model.Company_History>(cp);
                        if (flag)
                        {
                            Common.MessageBox.ShowRedirect(this, "/AboutUs/DevelopmentHistory.aspx");
                        }
                    }
                }
                else
                {
                    Model.Company_History cp = new Model.Company_History();
                    cp.Language = Convert.ToInt32(Request.Form["language"]);
                    cp.Title = txtTitle.Text;
                    cp.Content = XiangQing;
                    cp.CreatePeople = txtCreatePeople.Text;
                    ...
                    bool flag = Factory.GetExecution().Add<Model.Company_History>(cp);
                    if (flag)
                    {
                        Common.MessageBox.ShowRedirect(this, "/AboutUs/DevelopmentHistory.aspx");
                    }
