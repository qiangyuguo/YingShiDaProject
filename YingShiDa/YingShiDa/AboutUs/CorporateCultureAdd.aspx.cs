...
{
    public partial class CorporateCultureAdd : PageBase
    {
        ...
        public void BindData()
        {
            ...
            {
                Model.Company_Culture cp = Factory.GetExecution().SelectByID<Model.Company_Culture>(requestID);
                if (cp != null)
                {
                    if (cp.Language == 1)
                    {
            ...
            try
            {
                ...
                if (view_action == "notify")
                {
                    ...
                    if (cp != null)
                    {
                        ...
                        bool flag = Factory.GetExecution().Update<Model.Company_Culture>(cp);
                        if (flag)
                        {
                            Common.MessageBox.ShowRedirect(this, "/AboutUs/CorporateCulture.aspx");
                        }
                    }
                }
                else
                {
                    Model.Company_Culture cp = new Model.Company_Culture();
                    cp.Language = Convert.ToInt32(Request.Form["language"]);
                    cp.Title = txtTitle.Text;
                    cp.Content = XiangQing;
                    cp.CreatePeople = txtCreatePeople.Text;
                    ...
                    cp.Keywords = txtKeywords.Text.Trim();
                    cp.Description = txtDescription.Text.Trim();
                    bool flag = Factory.GetExecution().Add<Model.Company_Culture>(cp);
                    if (flag)
                    {
                        Common.MessageBox.ShowRedirect(this, "/AboutUs/CorporateCulture.aspx");
                    }
