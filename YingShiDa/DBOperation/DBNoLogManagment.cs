...
{
    public class DBNoLogManagment
    {
        NoLogDbHelperSQL sqlHelper;
        private string connectString;
        //配置文件默认连接字符串
        private static string defaultConnectStr = null;


        public DBNoLogManagment()
        {
            if (string.IsNullOrEmpty(defaultConnectStr))
                defaultConnectStr = PubConstant.ConnectionString();
            sqlHelper = new NoLogDbHelperSQL(defaultConnectStr);
        }

        public DBNoLogManagment(string connStrong)
        {
            connectString = connStrong;
            sqlHelper = new NoLogDbHelperSQL(connectString);
        }
